...
const CardNav = ({
  ...
  const calculateHeight = () => {
    ...
    const topBar = 60 // height of top bar
  ...
  return (
    <div
      className={`card-nav-container fixed left-1/2 -translate-x-1/2 w-[95%] max-w-[900px] z-[99] top-[1.2em] md:top-[2em] ${className}`}
    >
      <nav
        ref={navRef}
        className={`card-nav ${
          isExpanded ? 'open' : ''
        } block h-[60px] rounded-xl shadow-lg relative overflow-hidden will-change-[height]
      ...
      >
        ...
        <div
          ...
          } md:flex-row md:flex-wrap md:items-stretch md:gap-[12px]`}
          aria-hidden={!isExpanded}
        >
          {(items || []).map((item, idx) => (
            <div
              key={`${item.label}-${idx}`}
              className='nav-card select-none relative flex flex-col p-[12px_16px] rounded-[calc(0.75rem-0.2rem)] min-w-0 flex-1 h-auto min-h-[60px] md:h-auto md:min-h-0'
              ref={setCardRef(idx)}
            ...
            >
              ...
              <div className='nav-card-links flex flex-col gap-[2px] mt-2'>
